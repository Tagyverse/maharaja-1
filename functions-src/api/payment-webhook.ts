...
async function findAndUpdateOrderInFirebase(
  firebaseUrl: string,
  ...
  try {
    let orders: Record<string, any> | null = null;
    ...
    if (!orders) {
      ...
      for (const [key, value] of Object.entries(allOrders)) {
        if ((value as any)?.razorpay_order_id === razorpayOrderId) {
          orders[key] = value;
          break;
        }
      }
...
function extractPaymentInfo(webhookData: any): { orderId: string; paymentId: string } | null {
