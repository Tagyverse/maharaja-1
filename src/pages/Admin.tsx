...
export default function Admin() {
  ...
  useEffect(() => {
    const loadTelegramConfig = async () => {
      try {
        ...
        if (snap.exists()) {
          setTelegramConfig({ bot_token: '', chat_id: '', enabled: false, ...snap.val() });
        }
      } catch (e) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [reviewsSnapshot, offersSnapshot, settingsSnapshot] = await Promise.all([
        get(reviewsRef),
        get(offersRef),
        get(settingsRef)
      ]);
  ...
  const handleSettingsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const settingsData = {
        ...
        how_to_order_video_url: settingsFormData.how_to_order_video_url,
        min_purchase_count: Number(settingsFormData.min_purchase_count) || 1,
        updated_at: new Date().toISOString()
      };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const dataRefs = {
        ...
        // Branding override (SuperAdmin Rebrand tool)
        branding: ref(db, 'super_admin_config/branding'),
      };
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-3 sm:p-6 lg:p-8 mb-4 sm:mb-8">
          ...
          <div className="block lg:hidden mb-4">
            ...
            >
              ...
              <option value="video-sections">Video Sections</option>
              <option value="sections">Homepage Sections</option>
              <option value="card-design">Card Design</option>
              <option value="banner-social">Banner & Social</option>
              ...
              {isAdminAuthenticated && <option value="bill-customizer">Bill Design</option>}
              <option value="settings">Settings</option>
              <option value="publish">Publish to Live</option>
            </select>
          </div>
                ...
                <form onSubmit={handleOfferSubmit} className="bg-teal-50 rounded-2xl p-4 sm:p-6 mb-8 border-2 border-teal-200">
                  ...
                  <div className="mb-4">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 border-2 border-teal-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500"
                      rows={4}
                      placeholder="Get 50% off on all crackers this week only!"
                      required
                    />
                  </div>
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-xl p-6 border-2 border-rose-200">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <h3 className="text-lg font-bold text-gray-900 mb-2">How to Order Video</h3>
                          <p className="text-sm text-gray-600 mb-4">YouTube URL shown on the login dialog</p>
                          <input
                            type="url"
                            value={settingsFormData.how_to_order_video_url}
                            onChange={(e) => setSettingsFormData({ ...settingsFormData, how_to_order_video_url: e.target.value })}
                            placeholder="https://youtube.com/watch?v=..."
                            className="w-full px-4 py-3 border-2 border-rose-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 text-sm"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-xl p-6 border-2 border-amber-200">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <h3 className="text-lg font-bold text-gray-900 mb-2">Minimum Purchase</h3>
                          <p className="text-sm text-gray-600 mb-4">Minimum products required to place an order</p>
                          <input
                            type="number"
                            min="1"
                            value={settingsFormData.min_purchase_count}
                            onChange={(e) => setSettingsFormData({ ...settingsFormData, min_purchase_count: Number(e.target.value) || 1 })}
                            className="w-full px-4 py-3 border-2 border-amber-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 text-sm"
                          />
                          <p className="text-xs text-gray-500 mt-2">Customer must have at least this many items in cart to checkout</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl p-6 border-2 border-blue-200">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">Telegram Order Notifications</h3>
                    <p className="text-sm text-gray-600">Get instant order notifications on Telegram</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <label className="flex items-center gap-3 cursor-pointer bg-blue-50 px-4 py-3 rounded-xl border-2 border-blue-200">
                    <input
                      type="checkbox"
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      {telegramConfig.enabled ? 'Telegram Notifications Enabled' : 'Telegram Notifications Disabled'}
                    </span>
                  </label>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                    <p className="text-xs text-gray-500 mt-1">Use @userinfobot to find your chat ID</p>
                  </div>

                  <button
                    onClick={saveTelegramConfig}
                    disabled={telegramSaving}
                    className="flex items-center gap-2 bg-blue-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-colors disabled:opacity-50"
                  >
                    <Save className="w-5 h-5" />
