...
export default function PurchaseNotification() {
  ...
  return (
    <div
      className={`fixed bottom-6 left-6 z-40 bg-white rounded-2xl shadow-2xl p-4 max-w-sm border-2 border-brand-soft transition-all duration-500 ${
        isVisible ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'
      }`}
    >
      <button
        onClick={() => setIsVisible(false)}
        className="absolute -top-2 -right-2 bg-brand hover:bg-brand-dark text-white rounded-full p-1 transition-colors"
      >
        <X className="w-4 h-4" />
      </button>
