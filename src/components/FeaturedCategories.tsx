...
export default function FeaturedCategories({ onNavigate, onAddToCart, onCartClick, onProductClick }: FeaturedCategoriesProps) {
  ...
  return (
    <div className="py-12 bg-brand-soft">
      {categoriesWithProducts.map(({ category, products }) => (
        <div key={category.id} className="mb-16 last:mb-0">
          <div className="max-w-7xl mx-auto flex flex-col items-center">
            <div className="text-center mb-8">
              <h2 className="text-3xl md:text-4xl text-gray-900 mb-3 text-shimmer" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800, letterSpacing: '-0.03em' }}>
                {category.name}
              </h2>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-3.5 lg:grid-cols-4 lg:gap-4 w-full px-4">
              {products.map((product, index) => {
                ...
                return (
                  <div
                    key={product.id}
                    className="group bg-white rounded-xl border-2 border-gray-200 overflow-hidden cursor-pointer shadow-sm"
                    style={{ ...cardStyles.style, animationDelay: `${index * 80}ms` }}
                    onClick={() => onProductClick(product)}
                  >
                    <div className="relative aspect-[4/5] overflow-hidden bg-gray-50" style={cardStyles.imageStyle}>
                      <LazyImage
                    ...
                    <div className="p-2.5 sm:p-3">
                      <h3 className="text-gray-900 text-sm sm:text-base font-semibold leading-tight line-clamp-2 mb-1.5">
                        {product.name}
                      </h3>
                      ...
                      <div>
                        {inCart && qty > 0 ? (
                          <div
                            className="flex items-center justify-between bg-green-100 rounded-full border border-green-300 h-8 sm:h-9"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <button
                              onClick={(e) => {
                              ...
                              className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-gray-900 hover:text-red-500 transition-colors rounded-full"
                            >
                              <Minus className="w-3.5 h-3.5" />
                            </button>
                            <span className="text-xs sm:text-sm font-bold text-gray-900 min-w-[16px] text-center">{qty}</span>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleAddToCart(product, e);
                              }}
                              className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-gray-900 hover:text-gray-700 transition-colors rounded-full"
                            >
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1 bg-green-100 text-gray-900 rounded-full h-8 sm:h-9 text-xs sm:text-sm font-bold hover:bg-green-200 transition-colors active:scale-[0.97] border border-green-300"
                          >
                            <Plus className="w-3.5 h-3.5" />
                            <span>Add</span>
                          </button>
            ...
            <div className="text-center mt-8">
              <button
                ...
                className="bg-green-100 hover:bg-green-200 text-gray-900 px-8 py-3 rounded-full font-bold transition-colors border border-green-300"
              >
